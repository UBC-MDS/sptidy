...
When working with linear regression and kmeans clustering, it's nice to:
...
sptidy::tidy_lr() provides a tidy data frame that summarizes a fitted linear regression object lm(). The output data frame has 4 columns, describing coefficient estimates, standard error, t-statistics and p-values.
...
sptidy::augment_lr() augments the data frame with predictions and residuals from a fitted linear regression object lm(). The output data frame has additional 2 columns, describing prediction and residuals with respect to each observation. 

```{r}
my_lr <- lm(Employed~., data = longley)
sptidy::augment_lr(my_lr,longley[,1:6],longley["Employed"])
```

### tidy_kmeans(): clean summary output for kmeans()

sptidy::tidy_kmeans() provides a tidy data frame that summarizes a fitted kmeans clustering object kmenas(). The output data frame has 3 columns, describing cluster number, cluster center and number of points within each cluster. 
...
sptidy::augment_kmeans() augments the data frame with cluster assignment from a fitted kmeans clustering object kmeans(). The output data frame has additional 1 column, describing cluster assignment with respect to each observation. 
